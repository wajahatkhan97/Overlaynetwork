...
object lookupdata {
  ...
  def check_zone(x:Int,y:Int,store_nodes:mutable.HashMap[Int, String],actual_Cordinates:mutable.HashMap[String, ListBuffer[Tuple2[Int, Int]]]) = {
    /*
  ...
  def receive = {
    ...
    case create_zone_bootstrap(x,y,path)=>{
      ...
      if(count_nodes_in_bootstrapmap>0) {
        ...
        check_zone(x,y,store_nodes,actual_coordinate_zone) //returns the zone


        update_zones() //update the zone
